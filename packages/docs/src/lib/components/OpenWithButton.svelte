<script lang="ts">
  ...
  type Pkg = { name: string; url?: string; description?: string };
  type Metadata = {
    source?: string;
    sourceUrl?: string;
  ...
  let {
    ...
    metadata?: Metadata;
    /** Project metadata used for branding and LLM context. */
    pkg?: Pkg;
    /** Optional active layer for layer-split sources (e.g. `svg`/`canvas`/`html`). */
    layer?: string;
    example?: boolean;
  } = $props();

  const project: Pkg = $derived(pkg ?? { name: 'Documentation', url: page.url.origin });

  // For layer-split components, prefer the variant matching the active layer
  // (e.g. `Foo.svg.svelte` when SVG is selected). Falls back to the canonical source.
  const source = $derived((layer ? metadata?.sources?.[layer] : undefined) ?? metadata?.source);
  const sourceUrl = $derived(
    (layer ? metadata?.sourceUrls?.[layer] : undefined) ?? metadata?.sourceUrl
  );
  ...
  const llmBaseContext = $derived(
    `The following is a documentation page from ${project.name}${project.description ? ` (${project.description})` : ''}. The page URL for "${pageName}" is ${pageUrl}. Be ready to help answer questions about this page.`
  );

  const menuItems = $derived([
    // Component source (only when a source is available, or on a standalone example)
    ...(source || example
      ? [
          {
