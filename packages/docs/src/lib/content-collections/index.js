    ...
    const components = defineCollection({
        ...
        transform: async (doc, context) => {
            ...
            const subdirs = existsSync(componentsRoot)
                ? readdirSync(componentsRoot).filter((entry) => !entry.startsWith('.') && statSync(join(componentsRoot, entry)).isDirectory())
                : [];
            ...
            for (const dir of searchDirs) {
                const hit = readSource(componentsRoot, componentsSourceBase, `${dir}${path}/${path}.base.svelte`) ??
            ...
            if (primary && !primary.url.endsWith('.base.svelte')) {
                for (const layer of ['svg', 'canvas', 'html']) {
                    const variant = readSource(componentsRoot, componentsSourceBase, `${splitDir}${path}.${layer}.svelte`);
                    if (variant) {
                ...
                try {
                    ...
                    const walk = async (props = []) => {
                        for (const p of props) {
                            if (p.description)
                                p.descriptionHtml = await renderInline(p.description);
                            if (p.properties)
                                await walk(p.properties);
                        }
                    };
