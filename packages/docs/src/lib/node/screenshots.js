...
let IMAGE_SIZES = [800, 400, 240];
// Limit for testing (set to undefined to process all)
let TEST_LIMIT = undefined;
 ...
 */
function hasExampleChanged(component, example, filePath, checksums) {
 ...
 */
function screenshotExists(componentName, exampleName, mode) {
    // Check if all WebP sizes exist
    return IMAGE_SIZES.every((size) => {
        const screenshotPath = path.join(SCREENSHOTS_DIR, componentName, `${exampleName}-${mode}-${size}w.webp`);
 ...
 */
async function captureScreenshots(page, componentName, exampleName, modes) {
 ...
 */
function cleanupOrphanedScreenshots(validExamples) {
    console.log('Checking for orphaned screenshots...\n');
    // Create a set of valid example paths for quick lookup (all sizes and modes)
    const validPaths = new Set(validExamples.flatMap((ex) => IMAGE_SIZES.flatMap((size) => [
