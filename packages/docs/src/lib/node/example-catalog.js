 ...
 */
function extractComponentsFromExample(filePath, allComponents) {
    const components = [];
...
function extractModuleExports(filePath) {
    ...
    const descMatch = moduleContent.match(/export\s+(?:let|const)\s+description\s*=\s*['"`](.+?)['"`]/);
    if (descMatch)
 ...
 */
function getComponentExamples(componentName, allComponents) {
 ...
 */
export function generateComponentCatalog(componentName, catalogPath, allComponents) {
    ...
    if (fs.existsSync(catalogPath)) {
        try {
            ...
            const existingCatalog = JSON.parse(existingContent);
            // Check if content has changed (excluding updatedAt field)
            const contentUnchanged = existingCatalog.component === componentName &&
...
export async function writeExampleCatalogs({ componentsDir, examplesDir, catalogDir, title = 'Example Catalog Generator', logger = console }) {
