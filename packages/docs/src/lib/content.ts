...
export type ContentType = 'components' | 'utils' | 'guides' | 'reference';
export type ContentPaths = Partial<Record<ContentType, string>>;
...
const defaultContentPaths: Record<ContentType, string> = {
	components: '/src/content/components',
	utils: '/src/content/utils',
	guides: '/src/content/guides',
	reference: '/src/content/reference'
};

function getContentPath(type: ContentType, contentPaths?: ContentPaths): string {
	return contentPaths?.[type] ?? defaultContentPaths[type];
}
...
export function resolveExamplePath(
	...
	if (
		(type === 'guides' || type === 'reference') &&
		(path.startsWith('./') || !path.startsWith('/'))
	) {
		const relativePath = path.startsWith('./') ? path.slice(2) : path;
		return `${getContentPath(type, contentPaths)}/${relativePath}`;
	}
	if (path.startsWith('./')) {
...
export function extractExampleReferences(markdownContent: string): ExampleReference[] {
	...
	const matches = [
		...markdownContent.matchAll(componentRegex),
		...markdownContent.matchAll(mdcRegex)
	];
...
export function createContentLoaders<Metadata extends { content: string }>(options: {
	...
	async function getMarkdownComponent(
		...
		const resolver = options.modules[`${getContentPath(type, options.contentPaths)}/${slug}.md`];
	...
	async function loadExamplesFromMarkdown(
		...
		const pathExamples = refs.flatMap((ref) => {
			...
			return [
				{
					path: ref.path,
					resolvedPath: resolveExamplePath(ref.path, currentPath, type, options.contentPaths)
				}
			];
		});
		...
		await Promise.all(
			componentExamples.map(async (ex) => {
				const exampleType: ExampleType = type === 'utils' ? 'utils' : 'components';
				const loaded = await options.loadExample(ex.component, ex.name, exampleType);
				if (loaded) {
